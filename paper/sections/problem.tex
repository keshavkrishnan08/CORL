...
\paragraph{H2 — the gap is regime-dependent.} Long-horizon and contact-rich tasks should
show larger gaps than short, contact-light tasks. This is not a hunch: the compounding-error
bound~\eqref{eq:dev} predicts the gap grows along $(L^H-1)/(L-1)$ in horizon $H$ and closed-loop
expansiveness $L$, so the regime effect has a sign and a shape fixed in advance. We fit a linear mixed-effects model
